# 🎄 Christmas Fun App
*Trello board https://trello.com/b/Ml3cY7Nd/year-2-christmas-group-project-addictive-fun-app*

> **A festive, Christmas-themed addictive website built collaboratively** 🎅🎲

Welcome to **Christmas Casino**, a holiday-inspired web application where users can play classic casino-style games with a cozy Christmas twist. This project is a **group collaboration** created as part of our class coursework and is designed to showcase teamwork, front-end/back-end skills, and creative design — all wrapped in holiday cheer!

---

## ✨ Project Overview

Christmas Casino brings the excitement of addictive-style games into a **safe, educational environment**. No real money is used — players with **virtual currency (milk & cookies)** while enjoying snowy visuals, festive music, and seasonal animations.

🎁 **Key Goals:**

* Practice collaborative software development using GitHub
* Apply web development concepts learned in class
* Build an engaging, themed user experience
* Learn version control, issue tracking, and pull requests

---

## 🎮 Features

* 🎰 Christmas-themed casino games

  * Slot machines
  * Dice / roulette / card-style games
* 🎄 Festive UI with snow, lights, and holiday colors
* 💰 Milk & Cookies Virtual currency system 
* 🧑‍🤝‍🧑 Multiplayer or leaderboard features
* 🔊 Sound effects & music
* 📱 Responsive design for desktop & mobile

---

## 🛠️ Tech Stack

*(May vary depending on team decisions)*
* **Hosting:** Local XAMPP Installation 
* **Frontend:** HTML, CSS, JavaScript
* **Backend:** PHP 
* **Frameworks/Libraries:**
  * Bootstrap
* **Version Control:** Git & GitHub

---

## 🚀 Getting Started

To run this project locally:

```bash
# Clone the repository
git clone https://github.com/554961/addictive-app-project.git

# Navigate into the project directory
cd christmas-casino

# Open index.html or start the dev server
```

*(Add backend setup instructions here if needed)*

---

## 🎅 Team Collaboration Rules

To keep everything running smoothly:

* 🌿 Use branches **(NO direct pushes to `main`)**
* 🧾 Write clear commit messages
* 🔀 Open pull requests for review
* 🐞 Report bugs using GitHub Issues
* 🎨 Keep the Christmas theme consistent

---

## 📁 Project Structure

```plaintext
/
├── components/        
├── styles/           
├── staff/           
├── database_code/      
├── minigames/
├── pages/
├── customer/
├── index.php      # Main entry point
└── README.md      
```

---

## ⚠️ Disclaimer

🚨 **This project does NOT involve real money.**

All games use **virtual currency only** and are created strictly for **educational purposes**.

---

## 🎄 Future Ideas

* 🎁 Daily Christmas rewards
* 🏆 Leaderboards & achievements
* ❄️ Animated snowfall effects
* 🎶 Toggleable holiday music
* 🎨 More festive game skins

---

## Acknowledgements

Big thanks to:

* Our classmates & project teammates
* Our instructor for guidance
* Open-source libraries & assets used

---
